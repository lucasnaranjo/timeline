...
const svgHeight = '100vh'; // 50% of the viewport height
;
...
d3.csv("events.csv").then(data => {
  ...
  const events = svg.selectAll(".event")
    ...
    .attr("r", 5) // Base size of the dots
    .attr("fill", "black"); // Set color of the dots to black

    // Draw spiral path for each event
    events.append("path")
    .attr("id", (d, i) => `spiral-path-${i}`)
    .attr("d", () => createSpiralPath(5, 4, 100)) // Set a fixed size for initial path
    .style("fill", "none")
    .style("stroke", "none");
    ...
    .attr("startOffset", "0%") // Start the text 10% along the path
    .attr("fill", "white")
...
function zoomed(event) {
  ...
  // Update the event group (dots and text)
  svg.selectAll(".event")
    .attr("transform", d => `translate(${transform.applyX(x(d.date))},${height / 2})`)
  ...
  svg.selectAll(".event circle")
    .attr("r", 5*transform.k - 0.5*(transform.k ** 2) ); // Adjust the size based on the zoom level
